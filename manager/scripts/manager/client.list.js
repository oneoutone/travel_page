(function() {
    ...
    ClientListCtrl.$inject = ['$scope', '$state', '$timeout', 'toastr', 'httpService', '$stateParams'];

    function ClientListCtrl($scope, $state, $timeout, toastr, httpService, $stateParams){
        var vm = $scope;
        ...
        vm.fetchData = function(){
            ...
            httpService.getAllCompanyList({filter: vm.filter, status: status, size: 10, page: vm.page}, function(data){
                vm.list = data
            }, function(err){
                console.log(err)
            })

            httpService.getAllCompanyCount({filter: vm.filter, status: status}, function(data){
